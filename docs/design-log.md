...
**제안된 작업방식**: 회차 단위 진행. 컨텍스트 유지를 위해 회차별 핵심 내용(떡밥, 플롯 변화 등)을 요약하는 문서를 회차별로 만들거나 통합해서 볼 수 있어야 함. 회차 집필·교정은 무조건 서브에이전트 호출이 아닌 단일 세션으로 진행 — 원래 파이프라인 표의 `prose-writer`↔`editor-critic` 이중 서브에이전트 구상 대체.

**평가 결과 (1차, `harness-reviewer` 호출)**:

1. 목적적합성: 통과 — 맥락복원→비트확인→집필→자기교정→저장→요약누적 흐름이 목표를 빠짐없이 다룸.
2. 실행가능성: 통과 — 전 과정이 파일 읽기/쓰기와 텍스트 생성뿐이라 로컬에서 그대로 재현 가능. update 서브에이전트 호출도 기존 검증된 패턴 재사용.
3. 생산성: 반려 — `summaries.md`를 무한정 누적되는 단일 파일로만 설계해, 회차가 많아질수록(100화 이상 흔함) 매 회차 세션의 컨텍스트 소모가 선형으로 늘어남.
4. 일관성: 통과 — "단일 세션"(집필 위임 금지)과 기존 update 서브에이전트 관행(세계관/플롯/캐릭터 보완 제안)은 대상이 달라 모순 없이 공존. `prose-writer`/`editor-critic`을 안 만드는 것도 0단계 placeholder의 정상적 재설계.

**종합 판정**: 반려 (생산성).

**개선 반영**: `summaries.md`에 40화 초과 시 아카이빙 규칙(최근 20화 상세 + 이전 압축색인 + `summaries-archive/`로 분리) 추가. 떡밥 추적 표를 최소요건으로 신설해 매 세션이 산문형 요약 전체를 안 읽어도 미해결 떡밥을 표만으로 파악 가능하게 함. 자기교정 체크리스트를 "권장"이 아닌 "최소요건"으로 명문화.
