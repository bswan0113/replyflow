...
4. **자기교정** (최소요건 — 매 회차 반드시 점검):
   ...
   - **AI 냄새 판정** (`docs/style/ai-smell.md` 기준): 이 항목만 다른 항목과 성격이 다르다 — 통과할 때까지(최대 3회) 반려·재작성을 반복하는 게이트다. 판정 결과는 항목별 근거(인용문)와 함께 기록한다. 3회를 넘겨도 반려면 `docs/style/ai-smell.md`의 절차대로 사용자에게 진행 여부를 묻는다. **통과(또는 3회 상한 후 사용자 승인) 없이는 6번(저장) 단계로 진행하지 않는다.**
